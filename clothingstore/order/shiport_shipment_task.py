# In order/shiport_shipment_task.py
...
def process_shipment_for_order(order_id):
    ...
    # ... (Code for calculating dimensions and getting the service is correct) ...
    ...
    # ... (Code for building the payload is correct) ...
    cod_amount = 0 # and so on...
    items_payload = []
    ...
    payload = {
        ...
        "receiver_mobile": sanitized_phone, "receiver_name": order.full_name,
        ...
        "shipment_type": "Parcel", "type": "Parcel",
        "total_amount": float(service["total_charges"]), "total_weight": float(total_weight),
        "weight": float(total_weight), "length": float(max_length),
        "width": float(max_width), "height": float(total_height),
    ...
    try:
        ...
        if result.get("status"):
            ...
            # ✅ UPDATED: Use the new shipping_status field on SUCCESS
            ...
            # ✅ UPDATED: Use the new shipping_status field on FAILURE
            order.shipping_status = "SHIPMENT_FAILED"
            order.save()
        ...
        # ✅ UPDATED: Use the new shipping_status field on EXCEPTION
        order.shipping_status = "SHIPMENT_FAILED"
        order.save()
